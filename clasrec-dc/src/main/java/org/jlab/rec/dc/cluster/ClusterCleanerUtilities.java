...
public class ClusterCleanerUtilities {
	...
	public FittedCluster SecondariesRemover(FittedCluster clus, ClusterFitter cf) {
		...
		for(int i =0; i<6; i++) {
			...
			if(hitsInLayer.size() ==2) {
				...
				//System.out.println(" docaSum "+docaSum+" passingCut "+passingCut+" layer "+hitsInLayer.get(0).get_Layer());
	 ...
	 * If Nc=4, keep only the first and last hit in that column; if Nc > 4, keep the first 2 and
