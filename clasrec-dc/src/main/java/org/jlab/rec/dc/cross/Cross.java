...
public class Cross extends ArrayList<Segment> implements Comparable<Cross> {
	...
	public void set_CrossParams() {
		...
		double fux = 0.5*(val_sl1+val_sl2);
		double fuy = 0.5*wy_over_wx*(val_sl2-val_sl1);
		double fuz = 1;
		
		double fu = Math.sqrt(fux*fux + fuy*fuy + fuz*fuz);
		
		fux/=fu;
		fuy/=fu;
		fuz/=fu;
		
		Point3D fdirVec = new Point3D(fux,fuy,fuz);
